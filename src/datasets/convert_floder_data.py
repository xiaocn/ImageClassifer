...
def dataset_exists(tfrecord_dir, num_shares_dict, split_num):
  """
  判断tfrecord数据集是否存在
  ...
  for split_name in ['train', 'validation', 'test']:
    ...
    for shard_id in range(num_shard):
      ...
      output_filename = os.path.join(tfrecord_dir,
                                     '%s_%s_%05d-of-%05d.tfrecord' % (data_name, split_name, shard_id, num_shard))
...
def run(dataset_dir, tfrecord_dir, validation_dp=0.3, test_dp=0.1, split_num=1000):
  ...
  if dataset_exists(tfrecord_dir, num_share_dict, split_num):
